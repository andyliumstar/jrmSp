...
public class SkypeKitRunner implements Runnable {
	...
	public void startApp() {
		startApp(false, DefaultPort.getDefaultPort(), false, "/tmp/", "/tmp/", false, false);
	}

	public void startApp(boolean logging) {
		/*startApp(logging, SKYPEKIT_DEFAULT_PORT, false, "/tmp/", "/tmp/", false, false);*/
		startApp(logging, DefaultPort.getDefaultPort(), false, "/tmp/", "/tmp/", false, false);
	}
	 ...
	 * @see #isAppRunning()
	 */
	private void startApp(String options) {
		synchronized (this) {
			skypeKitOptions = options;
			Log.d(TAG, "skypeKitOptions = " + options);
    ...
    private void ensureNoExistingSkypeKitProcess()
    {
        try {
            String ls_str;
            Process ls_proc = Runtime.getRuntime().exec("ps");
	...
	public void run() {
		...
		String filename = "linux-armv7-skypekit-voicepcm-videortp";
		...
		Log.d(TAG, "pathName is exist? " + dist.exists());
...
/*				appProcess = Runtime.getRuntime().exec("chmod 777 " + file.getCanonicalPath());
				...
				Thread.sleep(2000);//for restart, sleep sometime then start.
				...
				while ((line = br.readLine()) != null) {
					Log.d(TAG, "skyptkit output:" + line);
				}
	...
	private synchronized void appStopped() {
		...
		SktUtils.getSkypeApp().onSkypeKitStoped();
	}
}
